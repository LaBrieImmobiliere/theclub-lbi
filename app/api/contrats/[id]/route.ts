...
// DELETE — admin only, pour nettoyer un contrat test ou cree par erreur.
// Supprime aussi les reconnaissances d honoraires liees (pas de cascade auto).
export async function DELETE(
  ...
  const contract = await prisma.contract.findUnique({
    where: { id },
    select: { number: true, ambassadorId: true },
  });
  if (!contract) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.honoraryAcknowledgment.deleteMany({ where: { contractId: id } });
  ...
  await auditLog(
    ...
    id,
    sessionUser.id,
    `Contrat ${contract.number} supprimé par ${sessionUser.name || "admin"}`,
  );

  return NextResponse.json({ success: true, number: contract.number });
}
